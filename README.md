...
- Persistence (Postgres for orders/logs, time-series DB optional)
    - Feature Store (optional): deterministic rolling stats & realized vol (in-house/Feast/Hopsworks)
...
Deterministic, reproducible features for strategies and risk models.

### Options

- In-house (default here): lightweight rolling features over close prices under `com.trading.service.persistence.featurestore`.
- Feast: production-grade feature store (stubbed integration provided).
- Hopsworks: managed feature store (stubbed integration provided).

### In-House Feature Store

Components:

- `FeatureStore`: API with ingestClose and getFeatures(symbol, ts)
- `InMemoryTimeSeriesRepository`: deterministic, ordered storage of close prices (per symbol)
- `InHouseFeatureStore`: computes
    - rolling_mean_close
    - rolling_std_close (sample std)
    - realized_vol = sample stdev(log returns) * sqrt(annualization)
- `RollingStats`: numeric helpers

Usage:

```java
TimeSeriesRepository repo = new InMemoryTimeSeriesRepository();
FeatureStore fs = new InHouseFeatureStore(repo, /*window*/ 20, /*annualization*/ 252.0);
fs.ingestClose("AAPL", System.currentTimeMillis(), 150.0);
// ... more ingests ...
Map<String, Double> f = fs.getFeatures("AAPL", System.currentTimeMillis());
Double realizedVol = f.get(FeatureKeys.REALIZED_VOL);
```

Test:

```bash
./gradlew test --tests "*InHouseFeatureStoreTest"
```

Roadmap:

- Plug into Tick/Bar ingestion for continuous updates
- Persist time series to disk/DB
- Add additional features (RSI, MACD, z-scores, realized skew/kurtosis)
- Provide Feast/Hopsworks concrete implementations
